...
VALUES ('Первая линия (SYSADMIN)', 'Техническая поддержка и общие вопросы', 'FIRST_AVAILABLE', NOW(), NOW(), 0),
       ('Линия 1С (DEV1C)', 'Специалисты по 1С', 'FIRST_AVAILABLE', NOW(), NOW(), 0),
       ('Линия разработчиков (DEVELOPER)', 'Сложные, нестандартные или эскалированные обращения', 'LEAST_LOADED', NOW(),
        NOW(), 0)
ON CONFLICT (id) DO UPDATE SET name            = EXCLUDED.name,
                               description     = EXCLUDED.description,
...
-----------------------------------------------------------
INSERT INTO tickets (title, description, status, priority, created_by_id, created_at, updated_at,
                     category_user_id, escalated, version)
VALUES
-- Тикеты от user1 (Сарипов А.Е., id = 10)
('Не могу войти в систему', 'После ввода логина и пароля появляется ошибка "Неверные данные".', 'NEW', 'HIGH', 10,
 NOW(), NOW(), 1, false, 0),
('Не приходит письмо с подтверждением', 'Зарегистрировался, но письмо с подтверждением не приходит уже 30 минут.',
 'NEW', 'MEDIUM', 10, NOW(), NOW(), 1, false, 0),

-- Тикеты от user2 (Канаев В.А., id = 11)
('Проблема с печатью документов', 'При попытке распечатать документ из личного кабинета выходит пустой лист.', 'NEW',
 'MEDIUM', 11, NOW(), NOW(), 1, false, 0),
('Не отображается история платежей', 'В разделе "Платежи" ничего не видно, хотя платежи были.', 'NEW', 'HIGH', 11,
 NOW(), NOW(), 1, false, 0),

-- Тикеты от user3 (Петенков С.С., id = 12)
('Ошибка при загрузке файла', 'При попытке прикрепить файл к заявке выдаёт "Недопустимый формат".', 'NEW', 'MEDIUM', 12,
 NOW(), NOW(), 1, false, 0),
('Не могу сбросить пароль', 'Нажала "Забыли пароль?", но письмо с ссылкой не приходит.', 'NEW', 'HIGH', 12, NOW(),
 NOW(), 1, false, 0)
